...
一开始把想法想的过于简单，以为一下子就能实现，但真上手起来才发现到处都是坑。
以及让AI做的最多的可能不是agent怎么编排，而是后端的一些校验工作，以及如何设计工具
（比如工具返回是否稳定——吐槽一下B站的AI字幕获取）以及最后想到直接给agent加上
可以进行图操作的工具。
对于非科班出身并且从未做过项目的我来说可以是一次全新的尝试，每天都需要判断新的内容，
学习新的知识，以及提出新的想法和AI进行讨论。
整体的vibe coding是快速的（不过我也没有看代码，可能写了一个可以运行的屎山吧），关键在于决策和人工验证以及防止AI越做越复杂。
未来agent的一个训练方向或许是自己发现有能力缺失，自己写了个标准的tool然后进行调用。
